...
class Qc_Map():
    ...
    def get_stat(self):
        ...
        for s_n_r_l in self.qc_list.keys():
            ...
            qc_stat_list = [str(x) for x in self.get_qc_stat(self.qc_list[s_n_r_l])]
            map_stat_list =[str(x) for x in self.get_map_stat(self.qc_list[s_n_r_l])]
            #print self.pjdir
            #print self.pj_info
            #print s_n_r_l
    ...
    def myprint(self):
        ...
        for key in sorted(self.my_stat.keys()):
            #print my_stat[key]
            print '\t'.join(self.my_stat[key])

    def print_to_file(self,f_out):
        #f_out = open(out_file,'a')
        f_out.write('#Pjdir\tTime\tQC stat\tProject No.\tSample\tNovo ID\tLibrary ID\tRun\tLane\tRaw reads\tError Rate\tQ20\tQ30\tMapped\tDup(%)\tAverage depth\n')
        for key in sorted(self.my_stat.keys()):
            #print self.my_stat[key]
            #print '\t'.join(self.my_stat[key])
            f_out.write('\t'.join(self.my_stat[key])+'\n')
        f_out.flush()
        #f_out.close()
...
class Start():
    def __init__(self):
        ...
        self.new_stat = ''
    def start(self):
        if self.argv["type"] == 'animal':
            if os.path.exists('{}/pn.txt'.format(self.pwd)):
                self.new_stat = Qc_Map(self.pwd,self.argv["pjno"],'human')
                self.new_stat.get_stat()
            else:
                self.new_stat = Qc_Map(self.pwd,self.argv["pjno"],'animal')
                self.new_stat.get_stat()
        elif self.argv["type"] == 'human':
            if os.path.exists('{}/pn.txt'.format(self.pwd)) == False:
                self.new_stat = Qc_Map(self.pwd,self.argv["pjno"],'animal')
                self.new_stat.get_stat()
                #except:
                #    self.new_stat = Qc_Map(self.pwd,self.argv["pjno"],'human')
                ...
                self.new_stat = Qc_Map(self.pwd,self.argv["pjno"],'human')
                self.new_stat.get_stat()
        elif self.argv["type"] == 'qc':
            self.new_stat = Qc_Map(self.pwd,self.argv["pjno"],'qc')
            self.new_stat.get_stat()

    def myprint(self):
        if self.out_file != False:
            self.new_stat.print_to_file(self.out_file)
        else:
            self.new_stat.myprint()

if __name__ == "__main__":
    argparser = argparse.ArgumentParser(description="one qc mapping stat")
    argparser.add_argument('-t','--type',help="project type",required = True)
    argparser.add_argument('-p','--pjno',help="project number",required = True)
    myargv = argparser.parse_args()
    pwd = os.popen('pwd').read().strip('\n')
    myargv={"type":myargv.type,"pjno":myargv.pjno}
    starter = Start()
    starter.pwd = pwd
    starter.argv = myargv
    starter.start()
