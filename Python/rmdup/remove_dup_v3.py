...
class sjm_job():
    ...
    def add_sh(self,name,memory,cmd_shell):
        ...
        my_job += '  sched_options -V -cwd -q hw.q,pbd.q\n'
        my_job += '  cmd_begin\n'
        my_job += '    sh {}\n'.format(cmd_shell)
        my_job += '  cmd_end\n'
        my_job += 'job_end\n'
...
class MyList():
    ...
    def dup_mk(self,sample_list,min_data=0):
        dup_stat = {}
        max1,min1,min_data = (float(self.max),float(self.min),int(min_data)*math.pow(10,9)/300)
        ...
        for key,sl_value in zip(self.sl.keys(),self.sl.values()):
            ...
            if dup_stat.has_key(sample):
                ...
                dup_stat[sample] = {'raw':raw,'clean_r':clean_reads,'raw_ndr':raw - clean_reads*dup,
                                    'clean_ndr':clean_reads*(1-dup),'old_d':dup,'dup_dr':clean_reads*dup}
        
        print 'Id\tF\tThisReads\tSample'
        for key in self.sl.keys():
            ...
            if self.slac_dup[key] != '-':
                if dup_stat[sample]['old_d'] <= self.slac_dup[key]:
                    ...
                    self.sl[key]['f'] = '-'
                    print 'a\t{}\t{}\t{}\t{}\t{}'.format(self.sl[key]['f'],self.sl[key]['new_d'],'-',dup_stat[sample]['raw'],key)
                else:
                    ...
                    print 'b\t{}\t{}\t{}\t{}'.format(self.sl[key]['f'],self.sl[key]['new_d'],int(this_reads),key)
                ...
                if dup_stat[sample]['raw'] <=  min_data + self.power*math.pow(10,9)/300:
                    self.sl[key]['new_d'] = dup_stat[sample]['old_d']
                    self.sl[key]['f'] = '-'
                    print 'c\t{}\t{}\t{}\t{}'.format(self.sl[key]['f'],self.sl[key]['new_d'],'-',key)
                elif dup_stat[sample]['dup_dr'] <= mindr + self.power*math.pow(10,9)/300:
                    self.sl[key]['new_d'] = dup_stat[sample]['old_d']
                    self.sl[key]['f'] = '-'
                    print 'd\t{}\t{}\t{}\t{}'.format(self.sl[key]['f'],self.sl[key]['new_d'],'-',key)
                    ...
                    this_reads = self.sl[key]['raw'] - dup_stat[sample]['old_d']*self.sl[key]['f']*self.sl[key]['clean_r']
                    print 'e\t{}\t{}\t{}\t{}'.format(self.sl[key]['f'],self.sl[key]['new_d'],int(this_reads),key)
                elif dup_stat[sample]['dup_dr'] > maxdr:
                    new_dup_dr = random.randint(int(mindr),int(maxdr))
                    self.sl[key]['new_d'] = float(new_dup_dr)/(new_dup_dr+dup_stat[sample]['clean_ndr'])
                    self.sl[key]['f'] = float(dup_stat[sample]['dup_dr'] - new_dup_dr)/dup_stat[sample]['dup_dr']
                    this_reads = self.sl[key]['raw'] - dup_stat[sample]['old_d']*self.sl[key]['f']*self.sl[key]['clean_r']
                    print 'f\t{}\t{}\t{}\t{}'.format(self.sl[key]['f'],self.sl[key]['new_d'],int(this_reads),key)
    ...
    def relister(self,sl_dup):
        ...
        f_in = open(sl_dup)
        f_out = open('{}/new_sample_list'.format(self.root_dir),'w')
        for line in f_in:
            if line[0] == '#':
                f_out.write(line)
            else:
    ...
    def mk_sh(self,sample_list_dup):
        ...
        for key,value in zip(self.sample_list.keys(),self.sample_list.values()):
            for key2,value2 in zip(value.keys(),value.values()):
                ...
                f_out.write('python /HWPROJ2/HW/wangjinhao/software/script/process/mk_rmdup.py \\\n')
        ...
        for key,value in zip(self.sample_list.keys(),self.sample_list.values()):
            ...
            for value2 in value.values():
                for sample in value2:
                    ...
                    lib_dir = '{}/{}/tmp/rmdup_{}/RemoveDup/{}'.format(self.root_dir,key,sample[-1][2:],sample[3])
...
if __name__ == "__main__":
    ...
    sub_mkdup = subpar.add_parser('mkdup', help=r'Calculate the value of "f" and generate "sample_list_dup"(keep data size to make dup to 0.10~0.15)')
    sub_mkdup.set_defaults(cmd='mkdup')
    sub_mkdup.add_argument('-d','--data_size',default=0,help=r'Minimum amount of data')
